...
async function loadPromoCodesFromBackend() {
  try {
    const response = await fetch("http://localhost:5000/promoCodes");
...
async function saveCartToBackend() {
  ...
  try {
    const cartsResponse = await fetch("http://localhost:5000/carts");
    const carts = await cartsResponse.json();
    const userCart = carts.find((c) => c.userId === user.id);

    if (userCart) {
      await fetch(`http://localhost:5000/carts/${userCart.id}`, {
        method: "PUT",
        headers: {
    ...
    if (user) {
      fetch("http://localhost:5000/carts")
        ...
        .then((carts) => {
          const userCart = carts.find((c) => c.userId === user.id);
          if (userCart) {
            fetch(`http://localhost:5000/carts/${userCart.id}`, {
              method: "DELETE",
            });
          }
        });
